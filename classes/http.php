...
function geturl($host, $port, $url, $referer = 0, $cookie = 0, $post = 0, $saveToFile = 0, $proxy = 0, $pauth = 0, $auth = 0, $scheme = "http", $resume_from = 0) {
        ...
        if ($saveToFile) {
                ...
                if ($Resume ["use"] === TRUE) {
						...
						if($percent){echo '<script type="text/javascript"></script>';}
        ...
        do {
                ...
                if ($saveToFile) {
                        ...
                        if ($bytesReceived > $last + $chunkSize) {
								...
								if ($percent >= 0  ) { $rang_class = 'rafted'; } 
								if ($percent > 19.8) { $rang_class = 'raftec'; } 
								if ($percent > 36.7) { $rang_class = 'rafteb'; }
								if ($percent > 64.6) { $rang_class = 'raftea'; }
								if ($percent > 80.3) { $rang_class = 'raftez'; }
                                echo '<script type="text/javascript">pr('."'" . $percent . "', '" . $received . "', '" . $speed . "', '" . $rang_class . "');</script>";
                                $last = $bytesReceived;
                        }
                } else {
                        $page .= $data;
