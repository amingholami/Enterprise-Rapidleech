...
$nn = "\r\n";
$fromaddr = "RapidLeech";
$dev_name = 'RapidLeechHost.com Special';
$rev_num = 'v3.20 Updated 28-July-2010';
$plusrar_v = '4.2';
...
define ('CREDITS', '&copy; Copyright <a href="" title="'.$options['site_title_description'].'">'.$options['site_title'].'</a> 2011'.$tarikhe_copy.' | All Rights Reserved<br/>{<a href="https://sourceforge.net/projects/enterprise-rl/" target="_blank" title="Enterprise is a new Rapidleech script template based on HTML5, CSS3 and jQuery.">Enterprise</a>} Template by <a href="http://www.amingholami.com" alt="AminGholami.com" target="_blank" >AminGholami.com </a> <span> </span>
</div>');
...
if (! $_GET ["filename"] || ! $_GET ["host"] || ! $_GET ["path"])
{
	...
	$LINK = trim (urldecode ($_REQUEST ["link"]));
	if (! $LINK)
	{
...
else
{
    ...
    <div color="<?php echo $amin_color; ?>" class="defult">
                <div class="_div2">
                  <div class="_div3">
                    <div class="onvan">Transloading</div>
                    <div class="neveshte">
                    	<div class="neveshte_inner" align="center" id="transload_bar">
                        <div id="ezafi">
     <?php
	...
	elseif ($file ["bytesReceived"] == $file ["bytesTotal"] || $file ["size"] == "Unknown")
	{
		...
		echo '<script type="text/javascript">' . "pr('100', '" . $file ["size"] . "', '" . $file ["speed"] . "', 'raftez')</script>\r\n";
		...
		echo "<script type=\"text/javascript\">$('#ezafi').animate({'width':'60%'},{duration:750}).slideUp('850');$('#info_ezafi').fadeIn('200'); document.title = 'Transloaded Successfully!';</script>";
		echo ('<form method="post" name="flist" action="' . $PHP_SELF . '">');
		echo ('<input type="hidden" name="files[]" value="' . $file ['date'] . '" /><br />');
		echo ('<div align="center" style="width:25%">');
		echo renderActions();
		echo ('</div>');
		echo ('</form>');}
