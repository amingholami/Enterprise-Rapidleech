...
switch ($_GET['ajax']) {
	case 'server_stats':
		if ($options['server_info'] && $options['ajax_refresh']) {
			...
			$AjaxReturn['InUsePercent'] = round($prozent_belegt,"3");
			$AjaxReturn['DiskSpace'] = ZahlenFormatieren($insgesamt);
//			$AjaxReturn['HSUP'] = round($total_size*100/$belegt,2);
			// CPU data
			if($cpu_percent<10){$cpu_percent2 = '0'.$cpu_percent;}
			else{$cpu_percent2 = $cpu_percent;}

			$AjaxReturn['CPULoad'] = $cpu_percent2;
			$AjaxReturn['CPUPercent'] = $cpu_percent;
